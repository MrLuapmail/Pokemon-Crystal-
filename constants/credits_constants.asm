; CreditsStringsPointers indexes (see data/credits_strings.asm)
	...
	const PAAD_TESTING
	const CRYSTAL_KAIZO
	const CRYSTAL_KAIZO_DEV
	const CRYSTAL_KAIZO_PLUS_STAFF
	const CKK_PROJECT_MANAGEMENT
	const LUAPMAIL
	const CKK_ASST_PROJECT_MANAGEMENT
	const MANNAT
	const CKK_LEAD_PROGRAMMER
	const KDLPRO
	const CKK_PROGRAMMERS
	const XAVION
	const DEKZEH
	const NOT_SUICUU
	const ZUILS
	const MARIL300
	const SILVION
	const CKK_GAME_TESTER_THEORYCRAFTER
	const QUESTIONABLE_SPECIMEN
	const REFINERY
	const REJE
	const RUNABUN
	const SAFETYMAN
	...
	const PCHAL
	const FAREWELL_MSG_1
	const FAREWELL_MSG_2
